...
import { FileStatusAPI, getFileStatusAPI, isFileStale, getStaleFiles } from './fileStatusAPI.js';
...
describe('FileStatusAPI', () => {
  ...
  beforeEach(() => {
    ...
    mockService = vi.mocked(FileTrackerService.prototype);
  });
  ...
  describe('getFileStatus', () => {
    it('should delegate to service', () => {
      ...
      expect(api['service'].getFileStatus).toHaveBeenCalledWith('/test/file.txt');
  ...
  describe('getStaleFiles', () => {
    it('should return stale files from service', () => {
      ...
      expect(api['service'].getFilesByStatus).toHaveBeenCalledWith(FileStatus.READ_STALE);
  ...
  describe('getCurrentFiles', () => {
    it('should return current files from service', () => {
      ...
      expect(api['service'].getFilesByStatus).toHaveBeenCalledWith(FileStatus.READ_CURRENT);
      expect(result).toEqual(mockEntries);
    });
  });
  ...
  describe('getAllReadFiles', () => {
    it('should return all read files (current + stale)', () => {
      ...
      vi.mocked(api['service']).getFilesByStatus
        .mockReturnValueOnce(currentEntries) // First call for READ_CURRENT
  ...
  describe('getFilesNeedingAttention', () => {
    it('should return stale and error files', () => {
      ...
      vi.mocked(api['service']).getFilesByStatus
        .mockReturnValueOnce(staleEntries)   // First call for READ_STALE
